				...
				<input type="hidden" id="userId" name="userId" value="${user.userId }"/>
					<table align="center">
						...
						<tr>
							...
							<td>
								<select id="buildId" name="buildId" style="width: 90px;">
									<c:forEach var="build" items="${buildList }">
										<option value="${build.buildId }" ${user.buildId==build.buildId?'selected':'' }>${build.buildName }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td><input type="text" id="roomName"  name="roomName" value="${user.roomName }"  style="margin-top:5px;height:30px;" /></td>
						</tr>
						<tr>
